...
#
#const DIRECTION = {
	#Vector2.LEFT: RoomEntry.ORIENTATION.EAST,
	#Vector2.RIGHT: RoomEntry.ORIENTATION.WEST,
	#Vector2.DOWN: RoomEntry.ORIENTATION.NORTH,
	#Vector2.UP: RoomEntry.ORIENTATION.SOUTH
#}
...
func _on_deep_checkpoint_entered(body: Node2D) -> void:
	if body is not Player or not isPendingEntry:
		return
